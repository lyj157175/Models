...
import utils
'''
1. bos->target_vocab_num output
2. sort, beam_size个，保存beam_size个概率，并且保存beam size的state
3. beam_size个->NETWORK->beam_size*target_vocab_num output 
4. sort beam_size次beam_size*beam_size state
5. sort beam_size*beam_size beam_size state 概率
6 beam_size个->network 装3步
7. if eos, result.
'''
class Beam(object):
